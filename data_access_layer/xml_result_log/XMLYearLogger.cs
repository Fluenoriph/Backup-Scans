...
class XMLYearLogger : BaseXMLSumsData
{
    ...
    public XMLYearLogger(XMLYearLogFile file, Dictionary<string, int> main_protocols_sums, Dictionary<string, int> simple_protocols_sums)
    {
        Sums_Sector_in = file.Document_in!.Element(XMLLogTags.SUMS);

        // Запись и сохранение.

        WriteSums(XMLLogTags.MAIN_SUMS, main_protocols_sums, ProtocolTypesAndSums.MAIN_SUMS);
