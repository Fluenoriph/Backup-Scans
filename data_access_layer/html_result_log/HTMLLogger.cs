...
abstract class HTMLSumsLevelLog
{
    ...
    public HTMLSumsLevelLog(XElement? sums_sector, string? period = null)
    {
        ...
        CreateSumsTypeTableString(XMLLogTags.MAIN_SUMS[0], ProtocolTypesAndSums.MAIN_SUMS[0]);

        // Создание строки ЕИАС суммы.

        CreateSumsTypeTableString(XMLLogTags.MAIN_SUMS[1], ProtocolTypesAndSums.MAIN_SUMS[1]);

        // Сначала проверка на наличие суммы обычных протоколов, а потом создание таблицы.

        var simple_protocols_sums_lcl = GetValidSumValue(XMLLogTags.MAIN_SUMS[2]);
    ...
    void CreateSimpleProtocolsSumsTable()
    {
        ...
        CreateSumsTypeTableString(XMLLogTags.SIMPLE_PROTOCOLS_SUMS[11], ProtocolTypesAndSums.NOT_FOUND_SUMS[0]);

        // Неизвестные.

        CreateSumsTypeTableString(XMLLogTags.SIMPLE_PROTOCOLS_SUMS[12], ProtocolTypesAndSums.NOT_FOUND_SUMS[1]);
